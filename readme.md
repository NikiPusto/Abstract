...
- Это делается git push но если вы просто введете эту команду он прогрузит наш файл 
в крайний открытый проект, по этому нам надо поменять адрес выгрузки 
- Команда для этого : 
```git remote add origin git@github.com:"your name"/"name your project"```
Но для начала проверим с каким проектом связаны мы сейчас
```git remote -v```
